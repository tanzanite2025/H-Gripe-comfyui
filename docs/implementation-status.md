...
| Colour pipeline: wide-gamut 16-bit working space + manual/model split — [`design/colour-pipeline.md`](design/colour-pipeline.md) | 🟡 Partial | **Core landed (P1–P3).** CMYK decode coverage ✅ (#180–#186). Canonical surface is now **16-bit ProPhoto** for profiled CMYK (#188–#190); the card/model/output boundary colour-manages **ProPhoto → sRGB**, while plain images / naive CMYK egress as an exact bit-narrow (byte-exact contract held). **P4** (manual-path 16-bit chain, staged P4a–P4e): P4a landed — `image_buffer` carries the 16-bit `WorkingImage` natively (8-bit consumers get the sRGB egress; behaviour unchanged). Remaining: **P4b–P4e** (16-bit card wiring + 16-bit PNG/TIFF-with-ICC output) + **P5** (Python-bridge parity). |
