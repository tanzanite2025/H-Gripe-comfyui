...
- 优先适配各种第三方 API / 远端模型服务，让用户通过节点自由组合 OpenAI、Gemini、Claude、Runway、Kling、Veo、Stability、Replicate 等能力；这不等于恢复 H-Gripe 自己的账号云端。
...
- 不为了通用性牺牲个人使用体验。
- 不做云端账号系统、多用户工作区和远程同步作为核心路径；工作流文件、API 凭据、历史记录和输出结果默认都走本地工作区。

## 本地工作区策略

H-Gripe 的第一阶段定位是个人本地工具，不应该为了账号、团队、多租户或云端同步引入额外复杂度。

- 保留 ComfyUI 旧的 `/users`、`/userdata`、`/settings` 路由作为兼容层，避免前端和现有流程断掉。
- `/users` 只返回本地工作区状态，不再创建账号。
- `comfy-user` 请求头不再决定用户目录，所有外部 HTTP 请求都映射到 `user/default`。
- 系统内部数据继续使用 `folder_paths.get_system_user_directory()` 这类内部 API，不通过 HTTP 用户目录暴露。
- 工作流同步优先通过导出/导入文件解决；如果以后真的需要云同步，也应只同步明确选择的工作流/配置，而不是恢复完整账号系统。
...
- 已实现 `custom_http` provider，支持通用 HTTP GET/POST、headers、query、JSON body、multipart 字段/本地文件上传、`credentials_ref` 本地凭据、`profile_ref` 默认配置、timeout、重试、2xx/4xx/5xx 状态分流、成功响应原始 bytes 落盘，以及 `async_job` 提交-轮询-结果下载流程；`async_job` 已支持取消上下文和可选 `cancel_url` / `cancel_url_path` / `urls.cancel` provider-native cancel。
...
- 已新增 ComfyUI 薄节点：`custom_nodes/hgripe_api_nodes.py`，当前提供 `H-Gripe Custom HTTP API`、`H-Gripe Custom HTTP Multipart API`、`H-Gripe Custom HTTP Async Job`、`H-Gripe OpenAI Compatible Text`、`H-Gripe OpenAI Compatible Image`、`H-Gripe OpenAI Compatible Image Edit`、`H-Gripe OpenAI Compatible Audio Speech`、`H-Gripe OpenAI Compatible Audio Text`、`H-Gripe OpenAI Compatible Vision` 和 `H-Gripe Replicate Run`，把参数组装成 `ApiTask` 后交给 Rust broker。
...
- `custom_http async_job` 已支持提交、轮询、成功/失败状态判断、最终结果 URL 下载和 `output_files` 落盘，并支持通过 `cancel_url`、`cancel_url_path` 或返回体 `urls.cancel` 对第三方 provider/API 远端异步任务发取消请求；这是后续接 Kling、Runway、Veo、Replicate 或本地 GPU 服务的通用基座。
- `replicate run` 已在通用 async 基座之上实现 Replicate provider：内置 `/v1/models/{owner}/{name}/predictions` 与 `/v1/predictions`（version 模式）两种创建入口、`urls.get` 优先的轮询、多输出 URL 下载，并支持 Studio 取消时调用 prediction cancel endpoint。后续应把它接入语义节点，而不是继续复制更多 provider-specific 节点。
...
- 当前 Studio 桌面端 Run / Run xN 已开始走 Rust `run_studio_graph`，直接吃同一个 `WorkflowGraph` JSON；TypeScript `runGraph` 保留为浏览器预览和测试参考实现。节点级进度已通过 `studio:graph-run` Tauri event 回传，并已有第一版 `cancel_studio_run`：可在节点之间取消，也可把取消 token 下沉到 broker/provider；`custom_http async_job` 和 `replicate run` 已能在轮询/等待期间调用第三方 provider/API 远端异步任务的 provider-native cancel endpoint。后续还需要补日志分级、更完整的错误详情和更多 provider 的原生取消适配。
...
这条路线不追求和完整 ComfyUI 做同一件事，而是保留节点式创作的优点，把重点放在第三方 API / 远端模型服务的统一调度、缓存、自动化和个人高频工作流上；H-Gripe 自身仍按本地个人工具来设计。
